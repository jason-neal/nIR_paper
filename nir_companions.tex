...
The list of observations obtained with CRIRES are provided in Table~\ref{tab:observations}. The number of artefacts are how many of the 32 individual detector nod spectra that were found to contain artefacts, see Section \ref{\label{subsubsec:reduction}}. The SNR is calculated with the formula \(\textrm{SNR} = \frac{\mu}{\sigma}\) where $\mu$ and $\sigma$ are the mean and standard deviation in the continuum of detector 2 between 2130 and 2134 (see Fig.~\ref{fig:spectral_example}.
